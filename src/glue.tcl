...
proc create-liger-menus {} {
    global fsCommands fsViews fsChartCommands fsChartViews 

    add-item-to-xle-menu \
	{command -label "Rewrite Semantics" \
	     -command "window-to-liger 0 $self" \
	     -doc "Derives Glue semantics premises."} \
	fsCommands
}
   ...
   fswindow-to-premises ".semantics" $window  \
    ...
    # Convert contents of window to semantics; display
    proc window-to-liger {packed window} {
	
	global ligerwindowtop
	global ligerDisplay

    set fsData [get-window-prop $window data]
   fswindow-to-liger ".rewrite_semantics" $window  \
	          $ligerDisplay 522+$ligerwindowtop
}
...
#Method for producing a semantic interpretation using LiGER

proc fswindow-to-liger {displaywindow window displaymode position} {
    ...
    set ligerfile tmp/liger_[pid].pl	
    set gswbfile tmp/gswb_[pid].pl
    ...
    #Set path and parameters of liger
    set ligerpath "jars/liger.jar"
    set parameters "-rf $defaultRewriteRules -i $prologfile -o $ligerfile"

    #Set up the command to call LiGER in accordance with the parameters set in xlerc
    set liger exec
    lappend liger java
    lappend liger -jar
    lappend liger $ligerpath
    set liger [concat $liger $parameters]
    lappend liger "-mc"

    #puts $liger
    #Evaluates the command defined above

    if { [catch { eval $liger } error] } {
	puts stderr "Failed to run transfer system. Error:\n $error"
    }


    if {$transferDebug == 1} {
	puts "#### Output of Liger: $ligerfile ($sentence) ####"
	set fp [open $ligerfile]
	set data [read $fp]
	puts -nonewline $data
	puts "+------------------------------------------+"
	close $fp
        } else {
	puts "Generating $ligerfile"
        }

    
   #set path and parameters of gswb
    set gswbpath "jars/gswb.jar"
    set inout "-i $ligerfile -o $gswbfile"
	...
	lappend gswb "-s"
        ...
        if { [catch { eval $gswb } error] } {
	puts stderr "Failed to run gswb. Error:\n $error"
    }
